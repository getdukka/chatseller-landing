  ...
  return (
    <header 
      className={`fixed top-0 left-0 right-0 z-50 transition-all duration-500 ease-in-out ${
        isScrolled 
          ? 'py-3 bg-white/95 backdrop-blur-md shadow-lg border-b border-rose-100/50' 
          : 'py-6 bg-gradient-to-r from-rose-50/80 via-pink-50/80 to-purple-50/80 backdrop-blur-sm'
      }`}
    >
      <div className="container mx-auto px-4 md:px-12 flex items-center justify-between">
        
        {/* Logo avec vraie image SVG */}
        <button 
        ...
        >
          ...
          <span className={`text-xs text-rose-600/70 ml-8 font-medium transition-all duration-300 ${
        ...
        {/* Navigation Desktop - SPÉCIALISÉE BEAUTÉ */}
        <nav className="hidden lg:flex items-center space-x-8">
          {navigationItems.map((item, index) => (
            <button
              key={index}
              onClick={() => scrollToSection(item.sectionId)}
              className="text-sm font-medium text-gray-700 hover:text-rose-600 transition-colors duration-200 flex items-center group relative"
            >
              {item.label}
              {item.icon}
            </button>
              ...
              {isLoggedIn ? (
                <Button 
                  className={`group relative overflow-hidden rounded-full bg-gradient-to-r from-rose-600 to-pink-600 hover:from-rose-700 hover:to-pink-700 transition-all duration-300 ${
                    isScrolled ? 'text-sm px-4 py-2' : 'text-sm px-6 py-3'
                  }`} 
                  asChild
                >
                ...
                <div className="flex items-center space-x-3">
                  <Button 
                    variant="ghost" 
                    className={`rounded-full hover:bg-rose-50 text-gray-700 hover:text-rose-600 transition-all duration-200 ${
                  ...
                  <Button 
                    className={`group relative overflow-hidden rounded-full bg-gradient-to-r from-rose-600 to-pink-600 hover:from-rose-700 hover:to-pink-700 shadow-lg hover:shadow-xl transition-all duration-300 ${
                      isScrolled ? 'text-xs px-4 py-2' : 'text-sm px-6 py-3'
                    }`} 
                    asChild
                  >
      ...
      {/* Mobile menu avec animations beauté - CORRECTION DU BUG */}
      <div className={`lg:hidden absolute bg-white top-full left-0 right-0 transition-all duration-300 ease-in-out ${
        mobileMenuOpen 
          ? 'opacity-100 translate-y-0 pointer-events-auto' 
          : 'opacity-0 -translate-y-4 pointer-events-none'  // CORRIGÉ: opacity-0 au lieu de opacity-100
      }`}>
          ...
          <nav className="container mx-auto px-4 py-6 flex flex-col space-y-4">
            ...
            {navigationItems.map((item, index) => (
              ...
              >
                ...
                <div className="text-xs text-gray-500 group-hover:text-rose-500 transition-colors">
                  {item.description}
                </div>
              </button>
            ))}
